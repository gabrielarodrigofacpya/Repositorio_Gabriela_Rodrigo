#Evidencia 2 cambio en los ids + cambio en los nombres apellidos
import json
import os
import sys
import re 
...
ARCHIVO_RESERVAS = "reservaciones.json"

clientes = {}
salas = {}
reservaciones = {}

cliente_id = itertools.count(1)
sala_id = itertools.count(1)                               #Contadores para ID
reservacion_id = itertools.count(1)

def cargar_datos():
    global clientes, salas, reservaciones, cliente_id, sala_id, reservacion_id

    if os.path.exists(ARCHIVO_CLIENTES):
        with open(ARCHIVO_CLIENTES, "r", encoding="utf-8") as f:
            clientes.update(json.load(f))
    if clientes:
        max_id = max(int(k) for k in clientes)
        cliente_id = itertools.count(max_id + 1)

    if os.path.exists(ARCHIVO_SALAS):
        with open(ARCHIVO_SALAS, "r", encoding="utf-8") as f:
            salas.update(json.load(f))
    if salas:
        max_id = max(int(k) for k in salas)
        sala_id = itertools.count(max_id + 1)

    if os.path.exists(ARCHIVO_RESERVAS):
        with open(ARCHIVO_RESERVAS, "r", encoding="utf-8") as f:
            data = json.load(f)
            for k, v in data.items():
                v['fecha'] = datetime.strptime(v['fecha'], "%Y-%m-%d").date()
            reservaciones.update(data)
    if reservaciones:
        max_id = max(int(k) for k in reservaciones)
        reservacion_id = itertools.count(max_id + 1)

def guardar_datos():
    with open(ARCHIVO_CLIENTES, "w", encoding="utf-8") as f:
        json.dump(clientes, f, indent=4, ensure_ascii=False)

    with open(ARCHIVO_SALAS, "w", encoding="utf-8") as f:
        json.dump(salas, f, indent=4, ensure_ascii=False)

#Convertir fechas a string antes de guardar
    data_para_guardar = {
        k: {**v, 'fecha': v['fecha'].isoformat()}
        for k, v in reservaciones.items()
    }
...
def generar_cliente_id():
    return str(next(cliente_id))

def generar_sala_id():
    return str(next(sala_id))

def generar_reservacion_id():
    return str(next(reservacion_id))
...
def registrar_nv_cliente():
    print("\n --- REGISTRAR NUEVO CLIENTE --- ")

    def es_nombre_valido(texto):  # Solo letras y espacios
        return bool(re.fullmatch(r"[A-Za-zÁÉÍÓÚáéíóúÑñ ]+", texto))

    # === Solicitar nombre con validación inmediata ===
    while True:
        nombres = input("Nombre del cliente: ").strip()
        if not nombres:
            print("El nombre es obligatorio.")
            continue
        if not es_nombre_valido(nombres):
            print("El nombre solo puede contener letras y espacios.")
            continue
        break

    # === Solicitar apellidos con validación inmediata ===
    while True:
        apellidos = input("Apellidos del cliente: ").strip()
        if not apellidos:
            print("Los apellidos son obligatorios.")
            continue
        if not es_nombre_valido(apellidos):
...
def registrar_nv_sala():
    print("\n --- REGISTRAR NUEVO SALA --- ")
    nombre_sala = input("Nombre de la Sala: ").strip()
    cupo_sala = input("Cupo de la sala (Numero entero): ").strip()
    try:
        cupo = int(cupo_sala)
        if cupo <= 0:
            raise ValueError()
...
def registrar_reservacion():
    ...
    reservaciones[folio] = {
        ...
        'fecha': d,
        'turno': turno,
        'nombre del evento': nombre_del_evento,
    }
    guardar_datos()
...
def editar_nombre_de_reservacion():
    ...
    while True:
        ...
        try:
            desde = fecha_reserva(desde_s)
            hasta = fecha_reserva(hasta_s)
            if desde > hasta:
    ...
    print("-" * 80)
    for folio, r in rows:
        print(f"{folio:<10} {r['fecha']:<12} {r.get('sala_id', 'N/A'):<6} {r['turno']:<10} {r['nombre del evento']:<30}")
...
print("Archivos JSON se guardarán en:", os.getcwd())
